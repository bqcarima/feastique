...
class VendorService(
    ...
    @Transactional(readOnly = true)
    fun getVendorByUsername(username: String): Optional<Vendor> {
        return vendorRepository.findByUsername(username)
    }

    @Transactional(readOnly = true)
    fun getVendorByPhoneNumber(phoneNumber: String): Optional<Vendor> {
    ...
    fun signup(vendorSignupDTO: VendorSignupDTO) {
        if(vendorSignupDTO.username?.let { getVendorByUsername(it).getOrNull() } == null) {
            if(vendorSignupDTO.defaultPhoneNumber?.let {getVendorByPhoneNumber(it).getOrNull()} == null) {
                ...
                address.region = Region.NORTHWEST
                ...
                address.longitude = vendorSignupDTO.latitude
