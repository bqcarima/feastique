...
class OrderController(
    ...
    fun cancelOrUpdateOrder(
        ...
        return ResponseEntity("Order ${orderUpdateDto.orderStatus.type} successfully.", HttpStatus.OK)
    }
    ...
    fun getAllOrders(
        ...
        securityUtility.validatePath(vendorId, userDetails)
    ...
    @GetMapping(
        path = [
            "/customers/{customerId}/orders/scroll",
            "/vendors/{vendorId}/orders/scroll"
        ]
    )
    fun scrollOrders(
    ...
    ) : ResponseEntity<WindowResponse<OrderResponse>> {
        val pathId = customerId ?: vendorId
        securityUtility.validatePath(pathId!!, userDetails)
        val window = orderService.scrollOrders(userDetails, status, cursor, size)
